...
# Add statistical feature for MaxTimeToNearestStation grouped by DistinctName
for agg_func in ["mean", "max", "min", "std"]:
    df[f"Distinct_MaxTimeToNearestStation_{agg_func}"] = df.groupby("DistinctName")["MaxTimeToNearestStation"].transform(agg_func)
